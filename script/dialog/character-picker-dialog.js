export class CharacterPickerDialog extends Dialog {
    ...
    activateListeners(html) {
        ...
        html.find('.crew-member').click(this.handleCharacterSelect.bind(this));
    }
    ...
    static async buildCharacterSelector(characters) {
        ...
        for (let i = 0; i < characters.length; i++) {
            ...
            html += await renderTemplate('systems/bounty-hunter-ttrpg/template/partial/crew-member.html', {crewMember: actor, noCharSheetLink: true});
        }
        return `<ol>${html}</ol>`;
    }
    ...
    static buildDivHtmlDialog(divContent) {
        return "<div class='flex row'>" + divContent + "</div>";
    }
}