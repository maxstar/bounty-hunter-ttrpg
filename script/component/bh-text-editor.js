...
export class BhTextEditor extends TextEditor {
    ...
    static _createChallengeLink(match, skillName, apCost, description) {
        ...
        // Update link data
        const displayName = (broken ? skill : game.i18n.localize(entity.name));
        data.skillName = displayName;
        data.description = data.description || game.i18n.localize('BH.CHALLENGE') + ': ' + displayName;
        data.icon = 'fas fa-cog';
        data.dataset = {entity: 'Item', id: broken ? null : entity.id, apCost: apCost, description: description, skillName: displayName};
    ...
    static async _onClickSkillChallengeLink(event) {
        ...
        const html = await renderTemplate("systems/bounty-hunter-ttrpg/template/chat/skill-challenge.html", {challenge: a.dataset});
        const chatData = {
            user: game.user._id,
